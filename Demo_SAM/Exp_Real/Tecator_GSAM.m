...
MSE_GSAM=zeros(NIter,1);RMSE_GSAM=zeros(NIter,1);RSSE_GSAM=zeros(NIter,1);
...
ftest=K_test*alpha;
MSE_GSAM(ii)= mean( (ftest - Ytest).^2./Ytest.^2);
RMSE_GSAM(ii)= mean( (ftest - Ytest).^2./Ytest.^2);
RSSE_GSAM(ii)=calcRSSE(Ytest,ftest);
figure;plot(alpha)
set(gca,'XTick',0:115:1500);
title('\alpha for Tecator data')
alpha1=alpha.*(abs(alpha)>=5);
beta=zeros(1,nG);
for i=1:nG
